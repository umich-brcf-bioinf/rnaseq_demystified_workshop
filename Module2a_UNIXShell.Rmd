...
```
$ ls workshop_data/
example.sam  fastqs  human_reference
```
...
$ ls -l
total 6624
-rw-rw-r-- 1 rcavalca rcavalca 3236531 Dec 13 16:12 example_R1.fastq.gz
-rw-rw-r-- 1 rcavalca rcavalca 3535293 Dec 13 16:12 example_R2.fastq.gz
-rw-rw-r-- 1 rcavalca rcavalca    1912 Dec 13 16:12 example_sra.fastq
```
...
Let's make a new directory for the file `example.sam` called `alignments`:
...
```
$ ls -F
alignments/  example.sam  fastqs/  human_reference/
```
...
Let's say that we want to move the file `example.sam` into the `alignments/example` folder we just created. We can do that with the move command `mv`:

```
$ mv example.sam alignments/example
```

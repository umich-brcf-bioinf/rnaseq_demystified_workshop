---
title: "Day 2 - Module 8: DESeq2 Initialization"
...
str(dds)
```
Right now, there are quite a few "empty" slots that will be filled in when we proceed with the model fitting. Before we do that, let's discuss how to make model choices.

## Making model choices

When deciding on the model to specify for the DESeq2 object, 

[Including **covariates** in a DESeq2 model](https://support.bioconductor.org/p/98700/)
...
> * *Covariates*: Additional sample attributes that should be accounted for in DE comparisons
> * *Design Formula*: 
